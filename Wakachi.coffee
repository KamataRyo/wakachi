...
#
# originated in http://chasen.org/~taku/software/TinySegmenter/tiny_segmenter-0.2.js
# coffeefy and modified by KamataRyo
class TinySegmenter
    constructor: ->
        patterns =
            '[一二三四五六七八九十百千万億兆]':'M'
    ...
    @ctype_ = (str)->
        for i, chartype in @chartype_
            if str.match chartype[0] then return chartype[1]
        return 'O'

    @ts_ = (v)-> if v? then v else 0

    @segment = (input)->
        if (input is null or input is undefined or input is '') then return []
...
# jQuery plugin
jQuery.fn.wakachi = (options)->
    new Wakachi options
        .bind this
        .mainContainer # method chain
